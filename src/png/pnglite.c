...
static size_t file_read(png_t* png, void* out,
						size_t size, size_t numel)
{
	size_t result;
	if(png->read_fun) {
		result =
			png->read_fun(out, size, numel,
						  png->user_pointer);
    }
	else {
		if(!out) {
			result =
				fseek(png->user_pointer,
					  (long)(size*numel), SEEK_CUR);
		}
		else {
			result = fread(out, size, numel,
						   png->user_pointer);
		}
	}
...
static size_t file_write(png_t* png, void* p,
						 size_t size, size_t numel)
{
	size_t result;
...
static int png_write_ihdr(png_t* png)
{
	...
	file_write(png,
			   "\x89\x50\x4E\x47\x0D\x0A\x1A\x0A", 1, 8);
...
void png_print_info(png_t* png)
{
	...
	printf("\tfilter:\t\t%s\n",
		   png->filter_method?
		   "unknown, this is not good":"adaptive");
	printf("\tinterlace:\t%s\n",
...
static int png_filter(png_t* png,
					  unsigned char* data)
{
	return PNG_NO_ERROR;
}

static int png_unfilter(png_t* png,
						unsigned char* data)
{
	unsigned i;
	unsigned pos = 0;
